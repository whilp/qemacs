...
## Phase 10: Remove configure script

- [x] Move build defaults into Makefile with ?= (overridable)
- [x] Generate config.h as a Makefile target
- [x] Make config.mak optional (-include)
- [x] Update Makefile.cosmo to pass CC/CFLAGS directly
- [x] Update tests/Makefile with defaults
- [x] Remove stale ifndef CONFIG_WIN32 in Makefile
- [x] Update CI workflow (no more ./configure step)
- [x] Delete configure script
- [x] Fix test_unix.c int overflow in usec tests
- [x] Commit

## Phase 11: Consolidate to single Makefile

- [x] Merge GNUmakefile + Makefile.cosmo into Makefile
- [x] Add cosmo/ci/release/install-cosmocc targets
- [x] Remove dead ifdef blocks (CONFIG_FFMPEG, CONFIG_PNG_OUTPUT, CONFIG_QSCRIPT, CONFIG_32BIT, TARGET_GPROF, TARGET_ARCH_X86, GCC_MAJOR)
- [x] Clean up libqhtml/Makefile (optional config.mak, remove dead blocks)
- [x] Update CI workflows to use `make ci` / `make release`
- [x] Delete GNUmakefile and Makefile.cosmo
- [x] Update CLAUDE.md
- [x] Commit
