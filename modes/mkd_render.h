...
typedef struct MkdRenderSpan {
    ...
    int end;            /* offset after closing marker */
    int content_start;  /* offset of content (after opening marker) */
    int content_end;    /* offset of content end (before closing marker) */
} MkdRenderSpan;

/*
 * Classify a line of text into a markdown block type.
 * `buf` is a NUL-terminated char32_t line (no newline).
 */
